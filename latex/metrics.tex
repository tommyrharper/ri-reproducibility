...
\caption{Merged nested-sampling likelihood for the R2D2 and CLEAN searches. Contours show where the failure objective (PolyChord log-likelihood) is high in observing-parameter space.}
...
\caption{Side-by-side likelihood comparison for merged R2D2 and CLEAN nested-sampling runs. The comparison separates the algorithms so shared failure regions and algorithm-specific failure regions can be inspected directly.}
