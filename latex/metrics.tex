...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../benchmarks/merged-r2d2-wsclean-posterior.png}
\caption{Merged nested-sampling posterior for the R2D2 and CLEAN searches. The plot shows where the sampled failure objective concentrates in the observing-parameter space, highlighting regions where the reconstructions are most likely to degrade.
White means the worst algorithm performance. Dark areas are the best configurations for the algorithms performance.}
\label{fig:merged-r2d2-wsclean-posterior}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../benchmarks/merged-r2d2-wsclean-posterior-side-by-side.png}
\caption{Side-by-side posterior comparison for merged R2D2 and CLEAN nested-sampling runs. The comparison separates the algorithms so shared failure regions and algorithm-specific failure regions can be inspected directly.}
\label{fig:merged-r2d2-wsclean-posterior-side-by-side}
\end{figure}
