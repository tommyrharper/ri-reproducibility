...
def self_check_source_offset() -> None:
    ...
    try:
        ...
        centre = image_dim() // 2
        # Halfway out, not the corner: at `fraction = 1` the corner is past
        # the last pixel of an even-width image and source_pixel() clamps it.
        halfway = cube_to_params(np.full(n, 0.75))
        scale = nominal_pixel_size_arcsec(halfway["start_frequency_hz"])
        header = {"CDELT1": -scale / 3600.0, "CDELT2": scale / 3600.0}
        sx, sy = source_pixel(
            header, centre, centre, halfway["source_l_arcsec"], halfway["source_m_arcsec"],
            image_dim(), image_dim(),
        )
        half = round(float(by_name["source_l_pixels"]["max"]) / 2.0)
        assert (abs(sx - centre), sy - centre) == (half, half), (sx, sy, half)
        away = float(by_name["source_l_pixels"]["max"])
