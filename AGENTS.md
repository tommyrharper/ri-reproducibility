...
- The `polychord` and `meqtrees` images both bake in files from `scripts/lib/nested_sampling` at build time (no live mount); after editing those files, rebuild every image that copies them (`scripts/build.sh polychord`, `scripts/build.sh meqtrees`) before running the PoC, or it silently runs the stale baked-in code. `simulate_point_source_ms.py` and `point_source_forest.py` live in the `meqtrees` image, everything else in `polychord` - except that the `meqtrees` build also bind-mounts `poc_common.py` to bake the MS skeleton cache, so a `PARAMETER_SPACE` change needs both images rebuilt.
