...
| `docs/nested-sampling-run-scaling.md` | What a bigger run costs, which is the question the whole optimisation effort was for: a ten-times-bigger `--nlive` costs **nothing** per evaluation (126.0 evaluations/second and 95.4% of worker-time accounted at `--nlive 600`, against 116.8 and 94.4% at `--nlive 60` - the *smaller* run is slower, because at a fixed `--max-ndead` it compresses further into the expensive corner), so a big search's wall clock is just its evaluation count over that rate. Carries the refreshed cost model (`70.7 ms + 4.58 us x visibilities`), the restoring-beam fit priced end to end at **8.7% of the `wsclean` binary** by `-no-fit-beam` over 76 interleaved replays (and `-beam-fitting-size` closed at 0.9%), the run-size planning table with the real ceiling on this host (~850k evaluations, `--nlive 1000 --num-repeats 50`, set by rank 0's 9.34 KB of resident memory per record at the end of the run), and **the close of the -29% w-gridding lever**: the max ignored-`w` phase error is above ducc0's own 1e-4 epsilon on 5962 of 5962 evaluations, so no lossless per-evaluation rule can ever skip it |
