...
| `docs/nested-sampling-ms-open.md` | The largest per-evaluation item outside the gridding arithmetic: WSClean re-opens the parent Measurement Set 15.7 times an evaluation and casacore attaches every subtable on each open, so the simulator deletes the five nothing reads - +14.9% evaluations per second over three interleaved pairs of real searches, bit-identical images - with the `-log-time` phase timeline that found it, the three-arm replay table and its null, why the drop cannot move into the cached skeleton, why `OBSERVATION` and `FEED` are the end of the road, and what a kept MS is no longer |
