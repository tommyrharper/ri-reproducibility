...
| bug | `./ri runs` unusable after a long run | 71k and 13k evaluation runs | Counting evaluations stat-ed every evaluation directory: 214s over 13k and 1838s over 71k on Lustre, against 0.1s to read the directory names, and the listing timed out past ten minutes. Fixed: a finished run carries `evaluation_count` in its summary's head, and a live one is counted from the one line per evaluation in `run.log` (exact on both runs measured); the directory scan is the last resort. The listing went from 6m42s to 1.1s |
