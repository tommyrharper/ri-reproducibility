...
memory. This is consistent with the existing result-preserving tradeoff being
...
The latest three-repeat asynchronous WSClean control measured **110.21
evaluations/second** (103.82, 111.34, and 110.21) at 20 ranks, with **142.4
ms/evaluation** by median, **121.9-128.4 ms/evaluation** in the image binary,
and **34.3-34.7 MB** peak worker memory. This is another host-variance refresh,
not a new runtime change.

The newest three-repeat asynchronous WSClean control measured **111.44
evaluations/second** (117.85, 111.44, and 105.25) at 20 ranks, with **137.6
ms/evaluation** by median, **121.4-122.3 ms/evaluation** in the image binary,
and **34.4-34.6 MB** peak worker memory. This remains a host-variance refresh,
not a new runtime change.

A fresh three-repeat asynchronous WSClean control measured **108.9
evaluations/second** (108.1, 108.9, and 114.1) at 20 ranks, with **140.4
ms/evaluation** by median, **120.9-132.0 ms/evaluation** in the image binary,
and **34.4-34.6 MB** peak worker memory. This is consistent with host variance
and does not justify a runtime change.

The latest three-repeat asynchronous WSClean control measured **105.96
evaluations/second** (105.96, 105.05, and 111.32) at 20 ranks, with **150.5
ms/evaluation** by median, **122.7-138.9 ms/evaluation** in the image
...
about **2.8% slower** and provides no memory benefit, so the 20-rank default
...
A fresh three-repeat asynchronous WSClean control measured **102.0
evaluations/second** (102.0, 92.3, and 105.5) at 20 ranks, with **142.8
ms/evaluation** by median, **129.1 ms/evaluation** in the image binary, and
**34.5-34.6 MB** peak worker memory. This control refresh remains within host
variance; image binary is still the dominant stage and no runtime change is
claimed.
...
The latest three-repeat asynchronous WSClean control measured **105.0
evaluations/second** (108.6, 105.0, and 101.0) at 20 ranks, with **146.5
ms/evaluation** by median, **130.9 ms/evaluation** in the image binary, and
**34.3-34.6 MB** peak worker memory. This remains normal host variance, not a
claimed code speedup; image binary remains dominant.
...
(0.5% faster). This is too small to justify changing the inference layout,
especially without an end-to-end result-preservation check.
...
| WSClean throughput control refresh | `./ri bench run wsclean --preset throughput --repeat 3` | 110.21 eval/s median (103.82, 111.34, and 110.21), 142.4 ms/eval, 121.9-128.4 ms image binary, and 34.3-34.7 MB peak worker memory; host-variance refresh |
