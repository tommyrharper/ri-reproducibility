...
One hundred twenty-seven profiling rounds cut WSClean from ~2.3 s to ~143 ms/evaluation.
...
The current async WSClean throughput check measures **105.7 +/- 3.0
evaluations/second** at 20 workers, 142.8 +/- 3.0 ms/evaluation, and 34.4 MB
...
speedup; the image binary remains the dominant stage at 127.4 +/- 3.0
ms/evaluation.
...
The 15-worker probe measured **105.1 +/- 3.3 evaluations/second** by the robust
median of three repeats, with 34.6 MB peak memory; it does not displace the
20-worker default.
...
The new 13-worker probe measured **51.5 evaluations/second** by the robust
median of three repeats, with 34.4 MB peak memory. It is substantially below
the 14-20-worker range and does not displace the 20-worker default.
The new 12-worker probe measured **94.05 +/- 3.8 evaluations/second** by the
robust median of three repeats, with 34.4 MB peak memory. Its 90.91 +/- 2.6
ms/evaluation image-binary stage is slower than the 14-20-worker range, so it
does not displace the 20-worker default.
The new 11-worker probe measured **82.25 +/- 2.1 evaluations/second** by the
robust median of three repeats, with 34.7 MB peak memory. Its 95.42 +/- 2.5
ms/evaluation image-binary stage is slower than the 12-20-worker range, so it
does not displace the 20-worker default.
...
ms/evaluation image-binary stage does not displace the 20-worker default.
The new 8-worker probe measured **73.85 evaluations/second** by the robust
median of three repeats (72.22, 73.85, and 79.03), with 34.4 MB peak memory.
Its 84.2 +/- 2.1 ms/evaluation image-binary stage is slower than the
9-20-worker range and does not displace the 20-worker default.
